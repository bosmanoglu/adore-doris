...
if isvector(off):
  ...
  mobj.master_timing.Coarse_correlation_translation_lines=(off[:,0]-off[:,2]).mean()-mobj.crop.First_line; #Average offset in azimuth
  mobj.master_timing.Coarse_correlation_translation_pixels=(off[:,1]-off[:,3]).mean()-mobj.crop.First_pixel; #Average offset in range  
